...
class build_transformer_att_PDCL(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_PDCL, self).__init__()
    ...
    def forward(self, x=None, label=None, get_image=False, get_text=False, cam_label=None, view_label=None):
        ...
        return text_features, img_feature_proj
...
class build_transformer_att(nn.Module):
    ...
    def forward(self, x=None, label=None):
        ...
        # FIXME 加入类别信息，做分类引导
        prompts = self.prompt_learner(label,[prompt_gender,prompt_upper,prompt_under,prompt_hair,prompt_shoes,prompt_bag])
        ...
        #         torch.Size([64, 1, 512])
        prompt_embedding = torch.cat([prompt_gender, prompt_upper, prompt_under, prompt_hair, prompt_shoes, prompt_bag]).squeeze()
...
class build_transformer_att_LAST(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_LAST, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
        ...
        dataset_name = cfg.DATASETS.NAMES
        # 这里修改prompt拼接
        self.prompt_learner = Prompt_Cat_7(num_classes, dataset_name, clip_model.dtype, clip_model.token_embedding)
        self.text_encoder = TextEncoder(clip_model)
        self.token_embbdings = clip_model.token_embedding
    ...
    def forward(self, x=None, label=None):
        ...
        prompt_bag = self.prompt_text_bag(img_f_clip).unsqueeze(dim=1)  #[64, 512]
        # FIXME 加入类别信息，做分类引导
        prompts = self.prompt_learner(label,[prompt_upper,prompt_under,prompt_hair,prompt_shoes,prompt_bag])
        text_f = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts)
        if self.model_name == 'RN50':
            img_feature = nn.functional.avg_pool2d(image_features, image_features.shape[2:4]).view(x.shape[0], -1)
        elif self.model_name == 'ViT-B-16':
        ...
        # print(prompt_bag.size())
        #         torch.Size([64, 1, 512])
        prompt_embedding = torch.cat([prompt_upper, prompt_under, prompt_hair, prompt_shoes, prompt_bag]).squeeze()
...
class build_transformer_att_LAST_2(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_LAST_2, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
        ...
        dataset_name = cfg.DATASETS.NAMES
        # 这里修改prompt拼接
        self.prompt_learner = Prompt_Cat_6(num_classes, dataset_name, clip_model.dtype, clip_model.token_embedding)
        self.text_encoder = TextEncoder(clip_model)
        self.token_embbdings = clip_model.token_embedding
...
class build_transformer_att_LAST_3(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_LAST_3, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
...
class build_transformer_att_LAST_4(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_LAST_4, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
        ...
        dataset_name = cfg.DATASETS.NAMES
        # 这里修改prompt拼接
        self.prompt_learner = Prompt_Cat_8(num_classes, dataset_name, clip_model.dtype, clip_model.token_embedding)
        self.text_encoder = TextEncoder(clip_model)
        self.token_embbdings = clip_model.token_embedding
...
class build_transformer_att_3(nn.Module):
    ...
    def forward(self, x=None, label=None):
        ...
        # FIXME 加入类别信息，做分类引导
        prompts = self.prompt_learner(label,[prompt_style,prompt_gender])
        text_f = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts)
        if self.model_name == 'RN50':
            img_feature = nn.functional.avg_pool2d(image_features, image_features.shape[2:4]).view(x.shape[0], -1)
        elif self.model_name == 'ViT-B-16':
        ...
        # print(prompt_bag.size())
        #         torch.Size([64, 1, 512])
        prompt_embedding = torch.cat([prompt_style,prompt_gender]).squeeze()
...
class build_transformer_att_4(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        dataset_name = cfg.DATASETS.NAMES
        # 这里修改prompt拼接
        self.prompt_learner = Prompt_Cat_4(num_classes, dataset_name, clip_model.dtype, clip_model.token_embedding)
        self.text_encoder = TextEncoder(clip_model)
        self.token_embbdings = clip_model.token_embedding
        self.prompt_text_semantic2 = IM2TEXT(embed_dim=clip_model.visual.output_dim,middle_dim=512,
                             output_dim=clip_model.transformer.width,
                           n_layer=3)
        self.prompt_text_semantic1 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                            output_dim=clip_model.transformer.width,
                                            n_layer=3)
        self.prompt_text_semantic3 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                            output_dim=clip_model.transformer.width,
                                            n_layer=3)
        self.prompt_text_semantic4 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                             output_dim=clip_model.transformer.width,
                                             n_layer=3)
        self.prompt_text_semantic5 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                             output_dim=clip_model.transformer.width,
                                             n_layer=3)
        self.prompt_text_semantic6 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                             output_dim=clip_model.transformer.width,
                                             n_layer=3)
    ...
    def forward(self, x=None, label=None):
        ...
        if self.model_name == 'RN50':
            ...
            img_f_clip = image_features_proj[:, 0]
        prompt_semantic1 = self.prompt_text_semantic1(img_f_clip).unsqueeze(dim=1)
        prompt_semantic2 = self.prompt_text_semantic2(img_f_clip).unsqueeze(dim=1)
        prompt_semantic3 = self.prompt_text_semantic3(img_f_clip).unsqueeze(dim=1)
        prompt_semantic4 = self.prompt_text_semantic4(img_f_clip).unsqueeze(dim=1)
        prompt_semantic5 = self.prompt_text_semantic5(img_f_clip).unsqueeze(dim=1)
        prompt_semantic6 = self.prompt_text_semantic6(img_f_clip).unsqueeze(dim=1)

        # FIXME 加入类别信息，做分类引导
        prompts = self.prompt_learner(label,[prompt_semantic1,prompt_semantic2,prompt_semantic3,prompt_semantic4,prompt_semantic5,prompt_semantic6])
        text_f = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts)
        if self.model_name == 'RN50':
            img_feature = nn.functional.avg_pool2d(image_features, image_features.shape[2:4]).view(x.shape[0], -1)
        elif self.model_name == 'ViT-B-16':
        ...
        # print(prompt_bag.size())
        #         torch.Size([64, 1, 512])
        prompt_embedding = torch.cat([prompt_semantic1,prompt_semantic2,prompt_semantic3,prompt_semantic4,prompt_semantic5,prompt_semantic6]).squeeze()
...
class build_transformer_att_5(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_5, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
        ...
        dataset_name = cfg.DATASETS.NAMES
        # 这里修改prompt拼接
        self.prompt_learner = Prompt_Cat_5(num_classes, dataset_name, clip_model.dtype, clip_model.token_embedding)
        self.text_encoder = TextEncoder(clip_model)
        self.token_embbdings = clip_model.token_embedding
        self.prompt_text_semantic2 = IM2TEXT(embed_dim=clip_model.visual.output_dim,middle_dim=512,
                             output_dim=clip_model.transformer.width,
        ...
        self.prompt_text_semantic7 = IM2TEXT(embed_dim=clip_model.visual.output_dim, middle_dim=512,
                                             output_dim=clip_model.transformer.width,
                                             n_layer=3)
    def forward(self, x=None, label=None):
        image_features_last, image_features, image_features_proj = self.image_encoder(x)
        ...
        prompt_semantic5 = self.prompt_text_semantic5(img_f_clip).unsqueeze(dim=1)
        prompt_semantic6 = self.prompt_text_semantic6(img_f_clip).unsqueeze(dim=1)
        prompt_semantic7 = self.prompt_text_semantic7(img_f_clip).unsqueeze(dim=1)
        # FIXME 加入类别信息，做分类引导
        prompts = self.prompt_learner(label,[prompt_semantic1,prompt_semantic2,prompt_semantic3,prompt_semantic4,prompt_semantic5,prompt_semantic6,prompt_semantic7])
        text_f = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts)
        if self.model_name == 'RN50':
            img_feature = nn.functional.avg_pool2d(image_features, image_features.shape[2:4]).view(x.shape[0], -1)
        elif self.model_name == 'ViT-B-16':
        ...
        # print(prompt_bag.size())
        #         torch.Size([64, 1, 512])
        prompt_embedding = torch.cat([prompt_semantic1,prompt_semantic2,prompt_semantic3,prompt_semantic4,prompt_semantic5,prompt_semantic6,prompt_semantic7]).squeeze()
...
class build_transformer_att_6(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        super(build_transformer_att_6, self).__init__()
        self.model_name = cfg.MODEL.NAME
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
        self.neck_feat = cfg.TEST.NECK_FEAT
...
def make_model(cfg, num_class, camera_num, view_num):
    if not cfg.MODEL.ATT:
        model = build_transformer(num_class, camera_num, view_num, cfg)
    else:
        model = build_transformer_att_LAST(num_class, camera_num, view_num, cfg)
    return model
...
class Prompt_Cat_7(nn.Module):
    def __init__(self, num_class, dataset_name, dtype, token_embedding):
        ...
        if dataset_name == "VehicleID" or dataset_name == "veri":
            ...
            ctx_init = "A photo of a person wearing X on top, " \
        ...
        self.register_buffer("token_prefix_2", embedding[:, :7, :])
