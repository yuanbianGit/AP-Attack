...
Imagenet_mean_G = [0.5, 0.5, 0.5]
Imagenet_stddev_G = [0.5, 0.5, 0.5]
inv_mean_G = [-m / s for m, s in zip(Imagenet_mean_G, Imagenet_stddev_G)]
inv_stddev_G = [1 / s for s in Imagenet_stddev_G]
from torchvision import transforms
def perturb_test(imgs, G,  train_or_test='test'):
  n,c,h,w = imgs.size()
  imgs = transforms.Compose([transforms.Normalize(inv_mean_G,inv_stddev_G),transforms.Normalize(Imagenet_mean,Imagenet_stddev)])(imgs)
  delta = G(imgs)
  delta = L_norm(delta, train_or_test)

  mask = torch.zeros_like(imgs).cuda()
  ...
  new_imgs = transforms.Compose(
      [transforms.Normalize(inv_mean, inv_stddev), transforms.Normalize(Imagenet_mean_G, Imagenet_stddev_G)])(new_imgs)
...
def L_norm(delta, mode='train'):
  ...
  return delta
G = Generator(3, 3, 32, norm='bn',beta=0.1).apply(weights_init)
G_checkpoint = torch.load('/data4/by/reid/github/MissRank_old/experiments/ours/meta/best_G.pth.tar')
    ...
    for n_iter, (img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            ...
            if cfg.MODEL.SIE_VIEW:
                ...
                target_view = None
            if index< num_query:
                new_imgs, delta, mask = perturb_test(img, G,  train_or_test='test')
                feat = model(new_imgs, cam_label=camids, view_label=target_view)
            else:
                feat = model(img, cam_label=camids, view_label=target_view)
            evaluator.update((feat, pid, camid))
            img_path_list.extend(imgpath)
